...
calculate-advisory-time: Converts times from the forecast advisory file to the date and hour.
calculate-coordinates: Reports lat-lon coordinates of the storm center in model space.
...
2. create-agents: Populates the model with the various breeds of agents (i.e., citizens, forecasters emergency managers, broadcasters, and aggregators). Sets various attributes for each citizen (i.e., evac-zone, self-trust, trust-authority, networks lists, risk thresholds). 
...
3. create-tract-agents: Populates the model with citizens based on census data. Other agents (i.e., forecasters, emergency managers, broadcasters, and aggregators) are populated similarly to create-agents.  
...
4. social-network: Assigns a social network for each citizen. Each citizen is also assigned a broadcaster and aggregator.  


go: This procedure moves the hurricane in the Netlogo interface, forecasters publish new forecasts, citizens receive the updated forecast and produces a mental model of the storm, emergency managers potentially issue evacuation orders, and citizens evaluate their risk to potentially make protective decisions. 
1. move-hurricane: Moves the hurricane symbol in the Netlogo interface. 
2. past-forecasts: Forecaster publishes a forecast modeled after the 5-day cone of uncertainty. A new forecast is published every 6 hours. 
...
4. coastal-patches-alerts: Coastal patches diagnose if their patch is within an intensity threshold and distance threshold to issue an alert. If so, the patch communicates with the EM to issue an alert.  
5. issue-alerts: The EM issues an evacuation order after coastal-patches-alerts issues an alert. 
...
demo-reload: Re-loads the model. Generates the storm and assigns new risk thresholds to each citizen. 
generate-storm: Translates the best track data to the model grid and interpolates storm characteristics to 1-hourly data. Currently, brute-force interpolation is used to convert 6-hourly data to 1-hourly data. Draws a line that represents the actual track of the storm.  
...
to go
  ...
  if ticks != 135 [ set clock list item 3 item ticks hurr-coords item 4 item ticks hurr-coords ]
...
to create-tract-agents
  ...
  ifelse limited-english-factor [ask cit-ags [set limited-english? add-census-factor 49]] [ask cit-ags [set limited-english? false]]
  ifelse use-food-stamps-factor [ask cit-ags [set food-stamps? add-census-factor 50]] [ask cit-ags [set food-stamps? false]]
  ifelse no-vehicle-factor [ask cit-ags [set no-vehicle? add-census-factor 59]] [ask cit-ags [set no-vehicle? false]]
  ifelse no-internet-factor [ask cit-ags [set no-internet? add-census-factor 55]] [ask cit-ags [set no-internet? false]]
...
to-report save-individual-cit-ag-evac-records
  ...
  ask cit-ags[
  set text-out (sentence ","who","xcor","ycor","kids-under-18?","adults-over-65?","limited-english?","food-stamps?","no-vehicle?","no-internet?","census-tract-number","evac_zone","completed","when-evac-1st-ordered","behaviorspace-run-number","which-storm?","distribute_population","earliest","wind_threshold","forc-w","evac-w","envc-w","network-distance","network-size","tract-information",")
  file-type text-out
  file-print ""
  ]
