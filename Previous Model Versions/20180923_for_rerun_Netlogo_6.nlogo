...
__includes ["./components/move-hurricane.nls" "./components/new-last-records.nls" "./components/coastal-patch-alerts.nls" "./components/issue-alerts.nls" "./components/dm.nls"]
...
gis:load-coordinate-system "/home/sbergin/CHIME/REGION/FLORIDA/GIS/block_density.prj"                  ; NetLogo needs a prj file to set up the conversion from GIS to netlogo grid
      ...
      set density gis:load-dataset "/home/sbergin/CHIME/REGION/FLORIDA/GIS/Pop_Density_1215.asc"       ; Raster map - Mean Annual Income (calculated by census tract 2015, modified for use w/ GRASS)

    set mean_income gis:load-dataset "/home/sbergin/CHIME/REGION/FLORIDA/GIS/median_income_FL_2011.asc" ; Raster map Mean HH Annual Income, calculated with census data from 2011. Split into 4 divisions, 4 is high income, 1 is low
...
to create-agents
  ...
  create-cit-ags #citizen-agents [
   ...
   [move-to one-of patches with [dens >= 0 ] ]
    ...
    ;;   *** SMB ***        9/25/18
    let income-here [income] of patch-here
    set income-here income-here * wealth-importance
  ...
  ;; for new decision model
    ...
    ;;   *** SMB ***        9/25/18
    ;; lower risk-life is more likely to move and wealthier income numbers are higher
    set risk-life risk-life - income-here
...
to social-network
  ...
  ask cit-ags [
    ...
    set total random-float sum [length network_list ^ net-power] of t-set
   let partner nobody
    if any? t-set [
     ask t-set [
...
;
;
;   let op (sentence "" which-storm? distribute_population earliest wind_threshold forc-w evac-w envc-w allpct "|" output-list-a "|" hist-list "|" hist-pcts "|")
;
...
@#$#@#$#@
<experiments>
  <experiment name="experiment_test" repetitions="1" runMetricsEveryStep="false">
    <setup>load-gis
load-hurricane
load-forecasts
setup
...
set output-filename "wilma_ideal"</setup>
  ...
  <experiment name="wealth_experiment" repetitions="500" runMetricsEveryStep="true">
    <setup>load-gis
load-hurricane
load-forecasts
setup
set output-filename "exp-test-wealth3"</setup>
    <go>go</go>
    <metric>new-last-records</metric>
    <enumeratedValueSet variable="latest">
      <value value="0"/>
    ...
    <enumeratedValueSet variable="wealth-importance">
      <value value="0"/>
      <value value="1"/>
      <value value="3"/>
      <value value="4"/>
  ...
  <experiment name="timer_test" repetitions="1" runMetricsEveryStep="false">
    <setup>load-gis
load-hurricane
load-forecasts
setup
reset-timer
set output-filename "timer-test"</setup>
    <go>go</go>
    <metric>new-last-records</metric>
    <enumeratedValueSet variable="latest">
      <value value="0"/>
  ...
  <experiment name="time_experiment" repetitions="2" runMetricsEveryStep="true">
    <setup>load-gis
load-hurricane
load-forecasts
setup
reset-timer
set output-filename "exp-test-time"</setup>
    <go>go</go>
    <metric>new-last-records</metric>
    <enumeratedValueSet variable="latest">
      <value value="0"/>
