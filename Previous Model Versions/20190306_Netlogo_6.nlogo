...
;    movie-close
  ...
  let x count cit-ags
  let xsum 0
  ask cit-ags [set xsum xsum + risk-watcher]
  let xavg xsum / x
  print xavg
...
to create-tract-agents
  ...
  let tractfile ""
  ifelse  using-hpc?
  [set tractfile "/home/sbergin/CHIME/flcensusdata/fltractpoint2.shp"
  print "using HPC file Location"
  ]
  [set tractfile "flcensusdata/FLTRACTpoint2.shp"]
  ...
  ;;Temporary for one test
  ask cit-ags [ set kids-under-5? false
    set adults-over-74? false]
    ;;Temporary for one test
  ifelse test-kids-under-5 [ask cit-ags [if test-frequency-under-5 > random 100 [set kids-under-5? true]]] [ask cit-ags [set kids-under-5? false]]
  ifelse test-adults-over-74 [ask cit-ags [if test-frequency-over-74 > random 100 [set adults-over-74? true]]] [ask cit-ags [set adults-over-74? false]]
...
;    let surge-info map [list item 0 ? item 1 item 1 ?] options
   ...
   ;; adds in environmental cues
    ...
    ;;New temporary add-on to test under-5 and over 74
...
which-storm?
which-storm?
"HARVEY" "WILMA" "WILMA_IDEAL" "CHARLEY_REAL" "CHARLEY_IDEAL" "CHARLEY_BAD" "IRMA"
6
